...
%token '+'  '-'  '*'  '/'  '%'  '='  '>'  '<'  '.'
%token ','  ';'  '!'  '('  ')'  '['  ']'  '{'  '}'
%token SUPER  DCOPY  SCOPY  COMPLEX  PRINTCOMP  CASE DEFAULT  DO  OD  CXXH
%token '@'  '#'  '$'  ':'
...
DoStmt          :   DO DoBranchs DoSubStmt OD
                    {
                        $$.stmt = new Tree.DosStmt($2.dolist, $3.ado, $2.loc);
                    }
...
DoBranchs       :   DoBranchs DoBranch
                ...
                |
                    {
                        $$ = new SemValue();
                        $$.dolist = new ArrayList<Tree.ADoStmt>();
                    }
...
DoSubStmt       :   Expr ':' Stmt
                    {
                        $$.ado = new Tree.ADoStmt($1.expr, $3.stmt, $1.loc);
                    }    
...
Expr            :	LValue
                ...
                |	'#' Expr
                	{
                		$$.expr = new Tree.Unary(Tree.TOCOMP, $2.expr, $1.loc);
                	}
                |	'$' Expr
                	{
                ...
                |	DCOPY '(' Expr ')' 
                	{
                		$$.expr = new Tree.DCopyExpr($3.expr, $3.loc);
                	} 
                |	SCOPY '(' Expr ')' 
                	{
                		$$.expr = new Tree.SCopyExpr($3.expr, $3.loc);
                	} 
                |   CASE '(' Expr ')' '{' ACaseExors DefaultExpr '}'
                	{
                		$$.expr = new Tree.CasesExpr($3.expr, $6.caselist, $7.dexpr, $3.loc);
                	}
                ;
...
DefaultExpr     :   DEFAULT ':' Expr ';'
                    {
                        $$.dexpr = new Tree.DefaultExpr($3.expr, $3.loc);
                    }
                ;
